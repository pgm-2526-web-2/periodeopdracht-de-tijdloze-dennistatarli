(() => {
  const API_URL = "https://www.pgm.gent/data/tijdloze/tijdloze.json";
  ...
  async function loadTop100() {
    try {
      ...
      let html = "";
      top100.forEach((song) => {
        const currentPosition = song[`position${year}`]; // Rank in 2024
        const previousPosition = song[`position${previousYear}`]; // Rank in 2023

        // Floating badge (rank)
        const badge =
          previousPosition === null
            ? "NIEUW" // Some songs have no ranking in 2023 -> list as "NIEUW"
            : previousPosition - currentPosition; // Calculate higher/lower ranking from song's ranking in 2023 and 2024
