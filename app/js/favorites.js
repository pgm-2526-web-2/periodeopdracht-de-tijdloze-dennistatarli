(() => {
  // Code runs only on page--favorites
  ...
  const renderFavorites = async () => {
    try {
      ...
      // Fetch (same as in tabs.js)
      const res = await fetch(API);
      const songs = await res.json();

      // Years (same as in tabs.js)
      const years = Object.keys(songs[0])
        .filter((key) => key.startsWith("position"))
        .map((key) => Number(key.replace("position", "")));

      // Get correct years (same as in tabs.js)
      const year = years[years.length - 1];
      const previousYear = years[years.length - 2];

      // Filters all songs to song with ID of favorite song
      const favSongs = songs.filter((song) => favIds.includes(song.song_id));

      // Render HTML
      let html = "";

      // Loop all favorite songs
      favSongs.forEach((song) => {
        // Same as in tabs.js
        const currentPosition = song[`position${year}`];
        const previousPosition = song[`position${previousYear}`];

        // Floating bagde (same as in tabs.js)
        let badge =
  ...
  <span class="list__rank-badge">${badge}</span>
      ...
      document.querySelectorAll("[data-remove-id]").forEach((btn) => {
        btn.addEventListener("click", () => {
          const id = Number(btn.dataset.removeId); // Convert string to number
          const favorites = getFavorites().filter((element) => element !== id); // Make new favorite list without the delete ID
          setFavorites(favorites); // Save this list in localStorage
          renderFavorites(); // Refresh favorite songs
        });
      });
    } catch (err) {
      console.error("Fetching favorite songs failed:", err);
    }
  };
